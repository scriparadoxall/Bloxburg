...
-- === SISTEMA SMART DOOR CORRIGIDO (Sem pausas, sem spam) ===
local lastDoorInteraction = 0
local function HandleNearbyDoors(hrp)
    -- Se apertou E há menos de 2.5 segundos, ignora (evita abrir e fechar a porta na cara)
    if tick() - lastDoorInteraction < 2.5 then return end 
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("Model") and (string.find(string.lower(obj.Name), "door") or string.find(string.lower(obj.Name), "porta")) then
            ...
            if distance < 5.5 then
                lastDoorInteraction = tick()
                -- task.spawn faz o clique acontecer sem pausar a caminhada
                task.spawn(function()
                    VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.E, false, game)
                    task.wait(0.05)
                    VirtualInputManager:SendKeyEvent(false, Enum.KeyCode.E, false, game)
                end)
                break -- Já achou uma porta, não precisa procurar outras agora
            end
        end
    end
end

-- === MOVIMENTAÇÃO INTELIGENTE LISA ===
local function MoveToStation(name)
    ...
    local allDoors = {}
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("BasePart") and obj.Parent and (string.find(string.lower(obj.Parent.Name), "door") or string.find(string.lower(obj.Parent.Name), "porta")) then
    ...
    for _, data in pairs(allDoors) do
        if data.Part then data.Part.CanCollide = data.OriginalCollide end
    end
    ...
    if success and path.Status == Enum.PathStatus.Success then
        ...
        for i, waypoint in ipairs(waypoints) do
            ...
            local t = 0
            -- Otimizado: delay de 0.05 pro boneco não frear nos waypoints
            while (hrp.Position - waypoint.Position).Magnitude > 2.5 and t < 40 do
                HandleNearbyDoors(hrp)
                -- Sistema anti-stuck melhorado
                if hum.MoveDirection.Magnitude < 0.1 and t > 5 then hum.Jump = true end
                task.wait(0.05)
                t = t + 1
            end
        end
    else
        Log("Aviso: Rota complexa falhou, indo em linha reta...", Color3.fromRGB(255, 150, 50))
        hum:MoveTo(walkPos)
        local t = 0
...
function _G.BloxburgChef.Start(food)
    ...
    Log("Farm Iniciado (Smart Navigate Fluido)", Color3.fromRGB(50, 255, 50))
        ...
        while isRunning and currentMacroId == myId do
            ...
            while isRunning and currentMacroId == myId and not hasFood do
                ...
                if startActText then
                    for _, vAct in pairs(validActions) do
                        if string.find(startActText, string.lower(vAct)) then
                            hasFood = true
                            Log("Comida já na mão!", Color3.fromRGB(50, 255, 50))
                            break
                        end
                    end
                end
                ...
                if MoveToStation("Icebox Fridge") then
                    Log("Aguardando UI da Geladeira...", Color3.fromRGB(200, 200, 200))
